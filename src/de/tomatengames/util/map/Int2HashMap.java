...
public final class Int2HashMap<V> implements Iterable<Int2Entry<V>> {
	...
	/**
	 * Creates a new and empty {@link Int2HashMap}.
	 */
	public Int2HashMap() {
		this.size = 0L;
		this.mask = 0;
	 ...
	 */
	public V put(int key1, int key2, V value) {
	 ...
	 * @throws IllegalArgumentException If the value is {@code null}.
	 */
	public V putIfAbsent(int key1, int key2, V value) {
		requireNotNull(value, "The value ...");
		Node<V> node = this.findOrCreate(key1, key2);
	 ...
	 */
	public V remove(int key1, int key2) {
	...
	@Override
	public void forEach(Consumer<? super Int2Entry<V>> action) {
		Node<V>[] table = this.table;
		if (table == null) {
	...
	public boolean equals(Object obj) {
		...
		// Checks the sizes.
		Int2HashMap<?> other = (Int2HashMap<?>) obj;
		...
		for (Int2Entry<?> entry : other) {
			V thisValue = this.get((int) entry.getKey1(), (int) entry.getKey2());
			if (thisValue == null) {
	...
	private static class Node<V> implements Int2Entry<V> {
		...
		@Override
		public int hashCode() {
			return Objects.hashCode(this.value) + (key2*31 + key1);
		}
	}
	
	
	@Override
	public Iterator<Int2Entry<V>> iterator() {
		return new NodeIterator();
	}
	
	private final class NodeIterator implements Iterator<Int2Entry<V>> {
