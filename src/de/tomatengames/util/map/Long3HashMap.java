...
public final class Long3HashMap<V> implements Iterable<Long3Entry<V>> {
	 ...
	 */
	public Long3HashMap(Long3HashMap<? extends V> map) {
		this();
		this.putAll(map);
	}
	 ...
	 */
	public void putAll(Long3HashMap<? extends V> otherMap) {
	...
	public boolean equals(Object obj) {
		...
		for (Long3Entry<V> entry : this) {
			Object otherValue = other.get(entry.getKey1(), entry.getKey2(), entry.getKey3());
			if (otherValue == null) {
	...
	private final Node<V> findOrCreate(long key1, long key2, long key3) {
		...
		int mask = this.mask;
		Node<V> node = findNode(key1, key2, key3, table, mask);
		if (node != null) {
			return node;
		}
		...
		insertNode(new Node<>(key1, key2, key3), table, mask);
